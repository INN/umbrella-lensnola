...
<?php
	...
	$category = get_category_by_slug('what-were-reading');

	$args = array(
		'post_type' => 'argolinksroundups',
		'category_name' => $category->slug,
		'paged'			=> $paged,
		'post_status'	=> 'publish',
		'posts_per_page'=> 1,
...
<?php
	...
	$args = array(
		'category_name' => 'squandered-heritage',
		'paged'			=> $paged,
		'post_status'	=> 'publish',
		'posts_per_page'=> 1,
		'post__not_in' 	=> $ids
	...
	if ( $wp_query->have_posts() ) {
		while ( $wp_query->have_posts() ) : $wp_query->the_post();
			...
			if ( in_array( get_the_ID(), $ids ) ) {
				...
				$ids[] = get_the_ID();
				get_template_part( 'content', 'squandered-heritage' );
			}
		endwhile;
	}
?>
